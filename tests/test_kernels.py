...
from tsipy.fusion.utils import build_sensor_labels
...
class TestWhiteKernel(unittest.TestCase):
    def test_kernel(self):
        ...
        labels, x_labels = build_sensor_labels((x_a, x_b, x_c))
        ...
        self.assertTrue(
            np.array_equal(
                linear_kernel(x).numpy().shape, multi_kernel(x).numpy().shape
            )
        )
        self.assertTrue(
            np.array_equal(
        ...
        # Test variances
        self.assertTrue(
            np.linalg.norm(multi_kernel(x).numpy() - np.diag(variances)) < 1e-9
        )
